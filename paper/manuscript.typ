...
Edge inference systems must preserve classification accuracy under restricted local computation and variable deployment conditions. Photonic architectures address this pressure by moving linear operations into optical propagation, while system designs divide storage and computation between remote, optical, and electronic components @Sludds2022NetcastEdge @Zhou2021ReconfigurableDPU. Their value depends on more than nominal accuracy: the inference path must expose which perturbations it tolerates, what representation crosses each computational boundary, and which performance claims come from hardware rather than simulation.

Diffractive deep neural networks implement learned transformations with passive phase layers and classify optical intensity at an output plane @Lin2018AllOpticalD2NN. Their physical structure introduces alignment, spacing, phase, detector, and quantization errors. Training-time misalignment sampling, phase filtering, parallel subnetworks, heterogeneous diffractive neurons, and sharpness-aware objectives provide established routes to robustness @Mengu2020MisalignmentResilient @Wang2025PhaseFilteredD2NN @Wang2026ParallelSubnetworkD2NN @Zhu2026HybridRobustODNN @Xu2026SharpnessAwarePNN. Hybrid optical-electronic classifiers provide another route by replacing fixed detector decisions with a learned electronic head @Mengu2020DiffractiveIntegration @Chang2018HybridOpticalElectronicCNN.
...
+ *Auditable evaluation.* We provide a seed-paired numerical protocol that connects full-test predictions, 14 deployment conditions, and integrity-checked summaries without treating samples or draws as independent replicates.
+ *Condition-specific robustness.* We show that perturbation-aware training redistributes accuracy toward displacement, mixed stress, and coarse quantization rather than uniformly improving every perturbation family.
+ *Bounded hybrid inference.* We show that a 64-element pooled optical representation and electronic head improve mean accuracy across most frozen conditions, while parameter counts and CPU timing remain model and simulator descriptors rather than hardware-efficiency evidence (Fig. 3).

= Related Work

== Diffractive classification and optical readout

Diffractive deep neural networks established that trained passive phase layers can implement classification and imaging through optical propagation @Lin2018AllOpticalD2NN. Subsequent work improved the readout by assigning positive and negative detector pairs to each class and by distributing classes across parallel diffractive networks @Li2019DifferentialDetection. These studies define the optical classifier and detector-readout families represented by our baseline D2NN. They do not compare a common seed-level cohort across clean inference, multiple deployment perturbations, and a compact electronic head.

== Hybrid optical-electronic and edge-oriented inference

Hybrid systems divide inference between an optical front end and an electronic back end. Jointly optimized diffractive layers and electronic classifiers showed that compressed optical outputs can feed lightweight electronic networks @Mengu2020DiffractiveIntegration. Optimized diffractive optics have also implemented the first layer of an optical-electronic convolutional classifier @Chang2018HybridOpticalElectronicCNN. Reconfigurable diffractive processors and hardware-software co-designed multi-task networks extend this idea to programmable and shared optical front ends @Zhou2021ReconfigurableDPU @Li2021RealtimeMultitaskD2NN. Netcast addresses a different edge architecture by streaming weights over deployed fiber to a low-storage photonic receiver @Sludds2022NetcastEdge. These systems motivate optical-electronic partitioning, but their physical latency and energy measurements do not transfer to our passive free-space simulator.

== Robustness of diffractive and physical neural networks

Robust diffractive training has several established forms. Misalignment-resilient networks sample lateral and axial offsets during training @Mengu2020MisalignmentResilient. Random phase dropout regularizes active diffractive models through sampled phase masks @Xiao2021RandomPhaseDropout. Phase filtering, parallel subnetworks, and random misalignment training improve tolerance to multi-plane alignment errors in imaging systems @Wang2025PhaseFilteredD2NN @Wang2026ParallelSubnetworkD2NN. Heterogeneous diffractive-neuron sizes provide a structural route that avoids conventional perturbation vaccination @Zhu2026HybridRobustODNN. At a broader physical-network level, sharpness-aware training optimizes deployment robustness without fixing one error distribution in advance @Xu2026SharpnessAwarePNN. Normalized cutoff frequency offers a complementary criterion for predicting robustness from propagated spatial-frequency content @Wang2026NormalizedCutoffRobustness. Our perturbation-aware D2NN uses the established vaccination strategy; the contribution lies in its bounded comparison against clean and hybrid alternatives, not in inventing perturbation injection.

== Quantization, coherence, and reproducible simulation

Phase-limited quantization-aware training directly optimizes D2NNs for discrete phase levels @Wang2025PLQAT, while two-level optical encoding has supported minimalist diffractive architectures @Liu2025MinimalistDONN. Spatial-coherence simulations show that fully coherent illumination forms a specific modeling boundary rather than a universal optical condition @Filipovich2024SpatialCoherence. LightRidge provides an open end-to-end framework for differentiable optical modeling and architecture exploration @Li2023LightRidge. Our study evaluates post-training phase quantization under a fully coherent scalar propagation model; it does not implement quantization-aware training or partial-coherence propagation.
...
#figure(
  image("figures/figure_2_complexity.svg", width: 100%, alt: "Model size and CPU software simulation boundary."),
  placement: top,
  caption: [*Model size and CPU software simulation boundary.* *a,* Trainable parameter counts and representation elements for the four methods. *b,* Clean accuracy against the median duration of a complete 10,000-sample CPU software forward for each training seed. Small points denote individual training seeds, and large points denote method means. Five timing repeats within each seed measured runtime precision and were not treated as independent replicates. CPU wall-clock values do not represent physical optical latency, edge-device latency, power, or energy efficiency.],
) <fig-complexity>
...
Five constraints limit the conclusions. First, MNIST does not represent natural images or task diversity. Second, three training seeds provide limited precision, especially for extreme quantization. Third, the fully coherent scalar propagation model omits partial coherence, material dispersion, fabrication calibration, and detector-system details; coherence alone can change diffractive-network behavior @Filipovich2024SpatialCoherence. Fourth, the study compares matched in-repository variants rather than reimplementing phase-filtered, structural-hybrid, sharpness-aware, or quantization-aware methods @Wang2025PLQAT @Wang2025PhaseFilteredD2NN @Zhu2026HybridRobustODNN @Xu2026SharpnessAwarePNN. Fifth, no physical optical platform or edge device validates latency, power, or energy claims.
...
#heading(numbering: none)[Ethics Statement]
