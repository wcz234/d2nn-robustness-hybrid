...
边缘推理系统需要在本地计算资源受限和部署条件不可控的双重约束下维持分类性能。与数据中心推理不同，边缘设备无法依赖稳定的机架环境和精密光学对准，温度漂移、机械振动和制造公差都会导致实际部署条件偏离设计标称值。光子计算架构提供了一条有吸引力的路径：将部分线性运算从电子处理器转移到光传播过程本身，通过无源相位层在物理空间中完成矩阵乘法，从而缓解边缘设备的存储和计算压力 @Sludds2022NetcastEdge @Zhou2021ReconfigurableDPU。然而，名义准确率不足以说明系统的实际适用性。一个在干净仿真条件下达到 89.64% 准确率的衍射网络，在 0.50 像素的层间错位下降至 85.33%，在四级相位量化下进一步降至 61.41%。完整的评估必须回答三个问题：模型能够容忍哪些扰动、哪种信息表示跨越光电计算边界、以及哪些性能结论来自数值仿真而非物理硬件。
...
衍射深度神经网络的鲁棒性研究已形成多条技术路线。Mengu 等人在训练阶段采样横向和轴向偏移，证明扰动注入可以提高对准容差 @Mengu2020MisalignmentResilient；Wang 和 Zhou 通过相位滤波和并行子网络结构增强了错位容忍度 @Wang2025PhaseFilteredD2NN @Wang2026ParallelSubnetworkD2NN；Zhu 等人提出异质衍射神经元尺寸，在不依赖扰动注入的条件下提供结构性鲁棒 @Zhu2026HybridRobustODNN；Hoshi 等人针对体全息衍射元件难以预先建模的波前像差，提出在训练阶段使模型适应未知像差的方法 @Hoshi2025WavefrontAberrationTolerant；Xu 等人将 sharpness-aware training 引入物理神经网络，从损失景观锐度的角度优化部署稳定性 @Xu2026SharpnessAwarePNN。在读出端，Mengu 等人分析了 D²NN 与电子网络的联合优化，证明学习型电子后端可以替代固定探测器判决 @Mengu2020DiffractiveIntegration；Chang 等人则用优化衍射光学元件承担光电卷积分类器的首层计算 @Chang2018HybridOpticalElectronicCNN。这些工作分别验证了鲁棒训练和混合架构的有效性，但存在一个共同的不足：每项研究使用自己的训练设置、评估条件和统计方法，结果之间无法直接比较。例如，Mengu 等人的错位鲁棒网络在特定错位分布下报告了准确率恢复，但没有说明同一模型在相位量化或探测器噪声下的表现是否同步改善；混合光电模型的研究通常只报告干净条件或单一扰动下的结果，缺乏多扰动联合作用的系统评估。

上述两条技术路线在读出设计上的差异进一步说明了这一不足。为每一类别配置正负探测器对、并将类别分配到并行衍射网络，可以在光强非负约束下改善读出对比度 @Li2019DifferentialDetection；可重构衍射处理器和软硬件协同多任务网络则把光学前端扩展为可编程、可共享的计算资源 @Li2021RealtimeMultitaskD2NN；主动调制衍射网络中的随机相位 dropout 通过采样相位掩模提供正则化 @Xiao2021RandomPhaseDropout。在建模边界方面，端到端可微光学设计框架使架构探索与器件约束可以在同一流程内完成 @Li2023LightRidge，而逐像素二级量化编码表明极简低比特光学前端同样可行 @Liu2025MinimalistDONN。相位受限的量化感知训练进一步说明，离散相位等级应在训练阶段建模而非事后施加 @Wang2025PLQAT；空间相干性研究则指出完全相干照明只是一种特定建模边界 @Filipovich2024SpatialCoherence，归一化截止频率从空间频率传播角度提供了鲁棒性的可解释判据 @Wang2026NormalizedCutoffRobustness。这些方法各自有效，但被报告的扰动族、读出结构和统计口径互不相同，因而无法回答本文关注的"收益出现在哪些条件下"这一问题。
...
+ *基准 D²NN*：采用干净条件训练，输出端使用十个固定探测器区域读出。该模型代表 D²NN 的标准配置，作为所有比较的参照基线。
+ *鲁棒 D²NN*：与基准 D²NN 共享相同的光学结构和读出方式，但在训练阶段采样部署扰动（详见 2.3 节）。该模型代表扰动感知训练策略。
+ *混合光电模型*：使用与基准 D²NN 完全相同的光学前端（三个可训练相位层），但输出端将光强进行自适应 $8 times 8$ 平均池化，形成 64 维表示，再输入 64-32-10 的电子 MLP 进行分类。该模型代表光电混合读出策略。
+ *纯电子基线*：采用 784-18-10 的多层感知机，直接将 $28 times 28$ 图像展平为 784 维输入，不含光学传播。该模型仅进入干净条件评估，用于提供电子计算的参考性能。
...
#figure(
  image("figures/figure_1_method_overview.svg", width: 100%, alt: "数值推理与训练路径。"),
  placement: top,
  caption: [*数值推理路径与扰动感知训练。* *a，* 数值推理路径。基准 D²NN 与鲁棒 D²NN 使用探测器区域能量读出；混合模型将 8×8 池化光强输入电子分类头；纯电子基线绕过光学传播直接处理图像向量。*b，* 训练阶段扰动采样。鲁棒 D²NN 在训练时采样横向错位、传播距离误差、相位噪声和相对探测器噪声；相位量化只用于部署评估，不进入鲁棒训练。全部路径均为标量数值仿真。],
) <fig-method-zh>

== 2.2 标量衍射传播模型

光学前端将输入图像嵌入 $64 times 64$ 振幅平面，并使用三个可训练相位层。本文采用与 Lin 等人 2018 年 D²NN 一致的数值尺度 @Lin2018AllOpticalD2NN：波长 0.75 mm，像素间距 0.4 mm，层间传播距离 30 mm。该尺度仅用于数值仿真，不对应本文制造或标定的实体器件。
...
每次传播执行带填充的线性卷积，并将中心区域裁剪回 $64 times 64$。层间距离误差直接改变相应传播算子的距离参数。
...
其中 $alpha=1$、$beta=0.1$、$gamma=0.01$。$cal(L)_("MSE")$ 比较归一化类别分数与 one-hot 标签，$cal(L)_("CE")$ 作用于模型 logits，$R_("phase")$ 为水平和垂直方向环形相位差平方的平均值。混合模型和纯电子基线使用交叉熵训练。
...
混合模型则把输出光强做自适应 $8 times 8$ 平均池化，得到 64 维表示后交由电子分类头处理，因此其读出边界与固定探测器掩模不同。
...
#figure(
  ...
  caption: [干净条件下的分类性能（均值 ± 标准差，$n=3$ 个训练种子）],
)
...
#figure(
  image("figures/figure_4_confusion.svg", width: 100%, alt: "干净条件的类别混淆结构。"),
  placement: top,
  caption: [*干净条件的类别混淆结构。* 四个面板分别对应基准 D²NN、鲁棒 D²NN、混合模型和纯电子基线。行表示真实数字，列表示预测数字；颜色表示三个独立训练种子的逐行归一化混淆矩阵均值，右侧色条范围为 0 至 1。每个面板标题中的准确率为三个训练种子准确率的均值。],
) <fig-confusion-zh>
...
#figure(
  ...
  caption: [鲁棒 D²NN 相对基准 D²NN 的配对准确率差（百分点，正值表示鲁棒训练更优）],
)
...
#figure(
  image("figures/figure_2_robustness.svg", width: 100%, alt: "协议约束的鲁棒性比较。"),
  placement: top,
  caption: [*预注册部署条件下的准确率。* *a，* 基准 D²NN、鲁棒 D²NN 和混合模型的平均准确率。*b，* 鲁棒 D²NN 和混合模型相对基准 D²NN 的配对准确率差。点表示三个独立训练种子的均值，误差条表示双侧 95% Student-t 边际置信区间。部署采样先在每个训练种子内聚合。本文未执行假设检验，也未对区间进行同时覆盖校正。],
) <fig-robustness-zh>
...
#figure(
  image("figures/figure_3_complexity.svg", width: 100%, alt: "模型规模与 CPU 软件仿真边界。"),
  placement: top,
  caption: [*模型规模与 CPU 软件仿真边界。* *a，* 四类方法的可训练参数量和表示元素数。*b，* 每个训练种子的干净准确率与完整 10 000 样本 CPU 软件前向中位时间。小点表示单个训练种子，大点表示方法均值。每个种子内的 5 次计时重复只用于估计运行时精度，不作为独立重复。CPU 软件仿真计时不代表物理光学时延、边缘设备时延、功耗或能效。],
) <fig-complexity-zh>
...
#figure(
  ...
  [59.51],
  [54.22 至 64.79],
  [57.75, 63.22, 63.27, 60.19, 53.11],
  [混合模型],
  [67.69],
  [50.13 至 85.24],
  [78.47, 84.28, 61.63, 48.54, 65.51],
  [鲁棒 D²NN],
  [77.56],
  [72.20 至 82.91],
  [81.21, 74.77, 81.99, 78.04, 71.78],
  [混合模型 + 量化感知训练],
  [94.10],
  [92.61 至 95.58],
  [94.29, 94.88, 92.02, 94.35, 94.94],
  ),
  caption: [扩展队列在四级相位量化下的准确率（均值，$n=5$ 个训练种子）],
)
...
#figure(
  ...
  caption: [扩展队列的干净条件准确率（均值，$n=5$ 个训练种子）],
)
...
#figure(
  ...
  caption: [Fashion-MNIST 上的干净条件与关键扰动条件（均值，$n=3$ 个训练种子）],
)
...
#figure(
  ...
  caption: [消融与相位滤波对照（均值，$n=3$ 个训练种子 42–44）],
)
...
但修复并不免费。量化感知训练使干净条件准确率降至 61.74%，说明模型为满足离散相位假设付出了连续相位下的表达能力。这一权衡与相位受限量化感知训练的既有结论一致 @Wang2025PLQAT：离散相位等级应在训练阶段建模，而建模的直接后果是训练分布与部署分布被绑定。因此本文的结论不是"量化感知训练更优"，而是：在本文的训练预算下，四级量化的失效属于训练过程问题；是否采用量化感知训练取决于部署时相位精度的确定性——若部署相位严格受限，则应当采用；若相位精度可协商，则连续相位训练给出更高的干净准确率。

== 4.2 混合模型优势的机制与边界

混合模型在 14 个条件中的 13 个条件下领先，其优势机制与鲁棒训练的扰动选择性不同。64 维池化强度保留了探测器区域读出（10 维）所丢失的空间细节，电子分类头因此能够学习到比固定线性探测器更灵活的类别边界。这一解释与 Mengu 等人关于 D²NN 与电子网络集成的研究一致 @Mengu2020DiffractiveIntegration，但本文的结果进一步表明，这种读出能力的提升在多类扰动下具有广泛性——无论是错位、噪声还是中等程度的量化，混合模型都保持了相对基准的优势。
...
第 3.5 节的量化感知训练结果修正了这一失效的归因。当同一混合架构在训练阶段即面对四级量化时，该条件的平均准确率升至 94.10%，且是该条件在四个方法、五个训练种子中的方差最小者。把两者放在一起看，未对量化建模的读出所表现出的种子依赖性，更可能是训练分布与部署分布不匹配的结果，而不是读出结构本身的不稳定性质。其代价是干净条件准确率降至 61.74%，说明这一修正把表达能力从连续相位重新分配到了离散相位，而非无条件提升。
...
*第二，混合光电读出的优势在多数条件下成立，极端量化是失效边界，且该失效主要来自训练过程。* 混合模型在 MNIST 与 Fashion-MNIST 上均在 14 个冻结条件中的 13 个条件下取得最高平均准确率，唯一例外都是四级相位量化。在同一混合架构上引入训练期四级相位量化后，该条件的平均准确率由 67.69% 提升到 94.10%（配对增益 26.41 个百分点，95% CI：9.26 至 43.56），并且是该条件在四个方法、五个训练种子中的方差最小者。因此该失效应表述为*未经量化建模的*混合模型在极端量化下的失效，而非混合架构的固有缺陷；其代价是干净条件准确率降至 61.74%，属于权衡而非无代价改进。
...
#bibliography("../references.bib", style: "ieee", title: [参考文献])
